...
async function handler(req: NextApiRequest, res: NextApiResponse<ResponseType>):Promise<void> {
  try {
    if(req.method === 'POST'){
      ...
      let checkEntry: EntryType| null = await Entries.findById(_id);
      if(checkEntry?.deleted_at === 'null'){
        return res.status(400).json({success: false, error: "Deletion failed", message: 'This entry does not exists in recyclebin'});
      }

      let entry: any | null = await Entries.findByIdAndDelete(_id);
      if(!entry){
      ...
      return res.status(200).json({success: true, message: 'Entry deleted permanently!'});
    }else{
